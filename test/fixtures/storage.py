...
@pytest.fixture
def test_in_memory_backend(test_config) -> InMemoryPolmanStorage:
...
def test_mongo_backend() -> MongodbPolicyStore:
    with mongomock.patch(servers=(("server.example.com", 27017),)):
        return MongodbPolicyStore(
            None,
            mongo_db_client=pymongo.MongoClient("server.example.com"),
            mongo_db_database_name="test",
        )
