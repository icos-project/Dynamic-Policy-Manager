...
async def get_validate_token(keycloak_client: KeycloakClientInstance, config: PolmanConfigInstance, authorization: Annotated[str | None, Header()] = None):
...
async def get_authenticated_user(keycloak_client: KeycloakClientInstance, config: PolmanConfigInstance, token = Depends(get_validate_token)):
    ...
    user = User(name=token_info['name'], email=token_info.get('email', ""), username=token_info['preferred_username'], scopes=token_info["scope"].split(" "))
    return user

async def get_authorized_user(request: Request, security_scopes: SecurityScopes, config: PolmanConfigInstance, keycloak_client: KeycloakClientInstance, token = Depends(get_validate_token), user: User = Depends(get_authenticated_user)):
