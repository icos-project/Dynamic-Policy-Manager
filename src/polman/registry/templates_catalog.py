...
POLICY_SPEC_TEMPLATES_CATALOG = {
    ...
    "app-sca-score": PolicySpecTelemetry(
        expr='tlum_workload_info{ {{subject_label_selector}} } * on(icos_host_id) group_left() (tlum_host_info * on(ip, hostname) group_left() label_replace(label_replace(SCA_score, "ip", "$1", "agent_ip", "(.+)"), "hostname", "$1", "agent_hostname", "(.+)"))',
        violatedIf="< {{threshold}}",
    ),
}
