...
import datetime
from polman.common.model import PolicyEvent, PolicyEventType, Violation
...
class PolicyEventsFactory:
  ...
  def policy_violated(violation: Violation):
    return PolicyEvent(type=PolicyEventType.Violated, timestamp=datetime.datetime.now(), details={
      "id": violation.id,
