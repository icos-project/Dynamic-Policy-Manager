...
def test_create_policies(test_http_client_mongo, policy_c1):
    response = test_http_client_mongo.post(
        "/polman/registry/api/v1/policies", json=policy_c1
    )
    ...
    assert (
        response.json()["status"]["measurementBackends"]["prom-1"]["rule_file"]
        == "test-rule.yml"
    )
    ...
    # test events
    assert len(p.status.events) == 3
    assert p.status.events[0].type == PolicyEventType.Created
    assert p.status.events[1].type == PolicyEventType.Rendered
    # true because currently we activate the policy when we create them
    assert p.status.events[2].type == PolicyEventType.Activated


def test_create_policy_no_activation(mocker, test_http_client_mongo, policy_c1):
    response = test_http_client_mongo.post(
        "/polman/registry/api/v1/policies?do_not_activate=True", json=policy_c1
    )
...
def test_unprocessable_policy(mocker, test_http_client, policy_c1):
    ...
    assert response.json()["detail"][0]["loc"][1] == "subject"
